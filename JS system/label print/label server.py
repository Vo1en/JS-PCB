...
def batch_print_gdi(printer_name, items):
    """
    使用 GDI 批量列印標籤
    items: List of (text, copies) tuples
    ★ 將所有項目合併到同一個列印工作 (Print Job) 中
    """
    try:
        ...
        # ★ 只呼叫一次 StartDoc
        hDC.StartDoc("Label Batch Print")
        ...
        for text_to_print, copies in items:
            # 針對每個項目計算字體大小 (雖然通常都一樣，但為了保險起見針對每個文字重算)
