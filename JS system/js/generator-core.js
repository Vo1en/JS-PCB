...
async function downloadPDF() {
    ...
    if (shippingMethod === "連片出貨" && (panelizingMethod === "工程代連片" || panelizingMethod === "工程代排版")) {
        ...
        panelDetailsText = `單片: ${singleX}x${singleY}mm | 排版: ${layoutX}x${layoutY} | 折斷邊: ${breakPosText} ${breakWidth}mm | 總尺寸: ${resultSize}`;
        ...
        if (sourceCanvas) {
            try {
                ...
                pdfPanelImg.src = sourceCanvas.toDataURL("image/png");
    ...
    try {
        ...
        pdf.save(`${partNo}_Spec_Form.pdf`);
